...
def check_separability(X, y, ε, canonical, solver, verbose=False):
    """
    ...
    # check results
    if feasible: assert (np.sign(w @ X.T) == y).all()

    return feasible, w

def check_separability_pooled(args):
    """
    ...
    # check feasibility
    feasible, w = check_separability(*args)
