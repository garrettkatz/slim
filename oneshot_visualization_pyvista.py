...
for all_planes in (False, True):
    ...
    for t,k in enumerate(K):
        ...
        # for x in xs:
            ...
            in_xs = (x == np.array(xs)).all(axis=1).any()
            if not all_planes and not in_xs: continue

            opacity = 1
            # opacity = 1 if in_xs else 0.5
        ...
        # plane intersections
        int_xs = X.T if all_planes else xs
...
if back:
    ...
    pt.figure(figsize=(2*6.5,2*2.5))

    for i, img in enumerate(imgs):
        # pt.subplot(1, len(imgs), i+1)
        pt.subplot(2, len(imgs)//2, i+1)
        pt.imshow(img)
        pt.axis('off')
        # pt.xticks([], [])
        # pt.yticks([], [])
